...
class EpayTest < Test::Unit::TestCase
  ...
  def test_successful_purchase
    Net::HTTP.any_instance.expects(:post).returns('Location' => valid_authorize_response)
  ...
  def test_failed_purchase
    Net::HTTP.any_instance.expects(:post).returns('Location' => invalid_authorize_response)
  ...
  def valid_authorize_response
    'https://ssl.ditonlinebetalingssystem.dk/auth/default.aspx?accept=1&tid=123&&amount=100&cur=208&date=20101117&time=2357&cardnopostfix=3000&fraud=1&cardid=18&transfee=0'
  end

  def invalid_authorize_response
    'https://ssl.ditonlinebetalingssystem.dk/auth/default.aspx?decline=1&error=102&errortext=The payment was declined. Try again in a moment or try with another credit card.'
  end
