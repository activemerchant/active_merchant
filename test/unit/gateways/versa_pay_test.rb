...
class VersaPayTest < Test::Unit::TestCase
  ...
  def test_build_order_request_url
    action = 'auth'
    assert_match %r{^#{Regexp.escape(@test_url)}/api/gateway/v1/orders/auth$}, @gateway.send(:url, action, :post)

    action = 'wallets'
    assert_match %r{^#{Regexp.escape(@test_url)}/api/gateway/v1/wallets$}, @gateway.send(:url, action, :post)
    assert_match %r{^#{Regexp.escape(@test_url)}/api/gateway/v1/wallets/WT/methods/FT$}, @gateway.send(:url, action, :delete, { fund_token: 'FT', wallet_token: 'WT' })

    # Test URL for 'wallets' action with missing tokens
    assert_match %r{^#{Regexp.escape(@test_url)}/api/gateway/v1/wallets//methods/$}, @gateway.send(:url, action, :delete)
  end
