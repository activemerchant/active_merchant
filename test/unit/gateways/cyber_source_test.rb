...
class CyberSourceTest < Test::Unit::TestCase
  ...
  def test_requires_error_on_tax_calculation_without_line_items
    assert_raise(ArgumentError){ @gateway.calculate_tax(@options.delete_if{|key, val| key == :line_items})}
  end
  ...
  def test_successful_store_request_with_cc
  ...
  def test_successful_store_request_with_check
    @gateway.stubs(:ssl_post).returns(successful_create_subscription_response)
    assert response = @gateway.store(@check, @subscription_options)
    assert response.success?
    assert response.test?
  end

  def test_successful_store_request_with_subscription
    @gateway.stubs(:ssl_post).returns(successful_create_subscription_response, successful_update_subscription_response)
    assert response = @gateway.store(@credit_card, @subscription_options)
    assert response.success?
    assert response.test?
    assert response = @gateway.store(response.authorization, @subscription_options.merge(:credit_card => @credit_card))
    assert response.success?
    assert response.test?
  end
