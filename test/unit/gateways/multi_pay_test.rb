...
class MultiPayTest < Test::Unit::TestCase
  ...
  def test_successful_access_token_fetch
    ...
    response = @gateway.send(:ensure_access_token)
    response = JSON.parse(response.body)
    assert_success response
    assert_equal 'test_access_token', @gateway.instance_variable_get(:@access_token)
  end
  ...
  def test_successful_capture
    ...
    assert_success response
    assert_equal '123456', response.authorization
    assert_equal 'Settlement successful', response.message
  end
  ...
  def test_successful_authorize_with_dollar_format
    stub_comms do
      ...
      if endpoint.include?('token')
        assert_match(/grant_type=client_credentials/, data)
      else
  ...
  def successful_capture_response
    {
      AuthorizeCaptureResponse: {
        Status: 'Success',
        AuthResultCode: '123456',
        ResponseMessage: 'Settlement successful'
      }
    }.to_json
  end
