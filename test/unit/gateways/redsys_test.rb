...
class RedsysTest < Test::Unit::TestCase
  ...
  def test_successful_purchase
    ...
    assert res.success?, "Transaction was not successful"
    assert_equal res.message, "Transaction Approved"
    assert_equal res.authorization, order_id
    assert_equal res.params['ds_order'], order_id
  end
  ...
  def test_failed_purchase
    ...
    res = @gateway.purchase(123, credit_card, :order_id => order_id)
    assert !res.success?
    assert_equal res.message, "Refusal with no specific reason"
    assert_equal res.params['ds_order'], order_id
  end
  ...
  def test_error_purchase
    ...
    res = @gateway.purchase(123, credit_card, :order_id => order_id)
    assert !res.success?
    assert_equal res.message, "SIS0051 ERROR"
  end
  ...
  def test_successful_refund
    ...
    assert res.success?
    assert_equal res.message, "Refund / Confirmation approved"
    assert_equal res.authorization, order_id
    assert_equal res.params['ds_order'], order_id
  end
  ...
  def test_error_refund
    ...
    res = @gateway.refund(123, order_id)
    assert !res.success?
    assert_equal res.message, "SIS0057 ERROR"
  end
  ...
  def test_authorize
    @gateway.expects(:post_and_parse).returns(true).with(includes("<DS_MERCHANT_TRANSACTIONTYPE>1</DS_MERCHANT_TRANSACTIONTYPE>")).with(includes("<DS_MERCHANT_PAN>4242424242424242</DS_MERCHANT_PAN>")).with(includes("<DS_MERCHANT_AMOUNT>123</DS_MERCHANT_AMOUNT>"))
  ...
  def test_capture
    @gateway.expects(:post_and_parse).returns(true).with(includes("<DS_MERCHANT_TRANSACTIONTYPE>2</DS_MERCHANT_TRANSACTIONTYPE>")).with(includes("<DS_MERCHANT_ORDER>1001</DS_MERCHANT_ORDER>")).with(includes("<DS_MERCHANT_AMOUNT>123</DS_MERCHANT_AMOUNT>"))
    @gateway.capture(123, '1001')
  end

  def test_void
    @gateway.expects(:post_and_parse).returns(true).with(includes("<DS_MERCHANT_TRANSACTIONTYPE>9</DS_MERCHANT_TRANSACTIONTYPE>")).with(includes("<DS_MERCHANT_ORDER>1001</DS_MERCHANT_ORDER>")).with(includes("<DS_MERCHANT_AMOUNT>123</DS_MERCHANT_AMOUNT>"))
    @gateway.void(123, '1001')
  end

  def test_override_currency
    @gateway.expects(:post_and_parse).returns(true).with(includes("<DS_MERCHANT_CURRENCY>840</DS_MERCHANT_CURRENCY>"))
  ...
  def test_overriding_options
    ...
    assert gw.test?
    assert_equal gw.send(:url), RedsysGateway.test_url
  end
