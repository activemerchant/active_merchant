...
class FirstdataE4V14Test < Test::Unit::TestCase
  include CommStub

  def setup
    @gateway = FirstdataE4V14Gateway.new(
      :login      => "A00427-01",
      :password   => "testus",
      :key_id     => 'KeyID',
  ...
  def test_successful_purchase
    ...
    FirstdataE4V14Gateway::SENSITIVE_FIELDS.each{|f| assert !response.params.has_key?(f.to_s)}
  end
  ...
  def test_supported_countries
    assert_equal ['CA', 'US'], FirstdataE4V14Gateway.supported_countries
  end

  def test_supported_cardtypes
    assert_equal [:visa, :master, :american_express, :jcb, :discover], FirstdataE4V14Gateway.supported_cardtypes
  end
  ...
  def test_requests_include_verification_string
    stub_comms do
      ...
      assert_match "<Address>456 My Street|K1C2N6|Ottawa|ON|CA</Address>", data
