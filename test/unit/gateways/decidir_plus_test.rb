...
class DecidirPlusTest < Test::Unit::TestCase
  ...
  def test_supported_card_types
    assert_equal DecidirPlusGateway.supported_cardtypes, %i[visa master american_express discover diners_club naranja cabal patagonia_365 sol]
  end
  ...
  def test_failed_refund_for_validation_errors
    response = stub_comms(@gateway, :ssl_request) do
      @gateway.refund(@amount, '12420186')
    end.respond_with(failed_credit_message_response)
  ...
  def failed_credit_message_response
    %{
