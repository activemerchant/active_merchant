...
class WorldpayTest < Test::Unit::TestCase
  ...
  def setup
   @gateway = WorldpayGateway.new(
    ...
    @options = {:order_id => 1}
    @reference_transaction_order_options = {:order_id => 1}
  end
  ...
  def test_successful_reference_transaction_authorize
    response = stub_comms do
      @gateway.authorize(@amount, @options[:order_id].to_s, @reference_transaction_order_options)
    end.check_request do |endpoint, data, headers|
  ...
  def test_successful_reference_transaction_authorize_with_merchant_code
    response = stub_comms do
      @gateway.authorize(@amount, @options[:order_id].to_s, @reference_transaction_order_options.merge({ :merchant_code => 'testlogin2'}))
    end.check_request do |endpoint, data, headers|
