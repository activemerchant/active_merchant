...
class RemoteClearhausTest < Test::Unit::TestCase
  def setup
    ...
    @mpi_options = { currency: 'DKK', order_id: '123', merchant: { id: '321', name: 'Best Merchant Inc', 
                     acquirer_bin: '411111', country: 'DK', url: 'http://myshop.com' } }
  end

  def test_successful_threed_auth
    options = @options.merge(order_id: '123', merchant: { id: '321', name: 'Best Merchant Inc', 
      acquirer_bin: '411111', country: 'DK', url: 'http://myshop.com' })

    response = @gateway.threed_auth(@amount, @credit_card, options)
  ...
  def test_invalid_mpi_login
    gateway = ClearhausGateway.new(api_key: 'test', mpi_api_key: 'test')

    assert_raise ActiveMerchant::ResponseError do
      gateway.threed_auth(@amount, @credit_card, @mpi_options)
    end
  end
end
