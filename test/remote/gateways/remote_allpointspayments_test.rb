...
class RemoteAllPointsPaymentsTest < Test::Unit::TestCase  

  def setup
    @gateway = AllpointspaymentsGateway.new(fixtures(:allpointspayments))
    @amount = 100
  ...
  def test_invalid_login
    gateway = AllpointspaymentsGateway.new(
    :login => '',
    :password => ''
    )
    assert response = gateway.purchase(@amount, @credit_card, @options)
