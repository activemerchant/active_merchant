...
class RemotePagarmeTest < Test::Unit::TestCase
  def setup
    ...
    @credit_card = credit_card('4242424242424242', {
      ...
      def test_successful_recurring_credit_card
        ...
        assert_nil response.params["current_transaction"][:boleto_barcode]
        assert_nil response.params["current_transaction"][:boleto_expiration_date]

        assert response.test?
      end

      def test_failed_recurring_credit_card_when_email_not_exists
        @options_recurring[:customer].delete(:id)
        @options_recurring[:customer].delete(:email)
        ...
        assert_false response.payment_action
        assert_equal 'email está faltando', response.message
        assert response.test?
      end
      ...
      def test_failed_recurring_credit_card_when_plan_code_is_invalid
        ...
        assert_false response.payment_action
        assert_equal 'Plano não encontrado.', response.message
        assert response.test?
      end

      def test_successful_recurring_boleto
        ...
        assert_not_nil response.params["current_transaction"][:boleto_expiration_date]

        assert response.test?
      end

      def test_failed_recurring_boleto_when_email_not_exists
        @options_recurring[:payment_method] = 'boleto'
        @options_recurring[:customer].delete(:id)
        @options_recurring[:customer].delete(:email)
      ...
      def test_invalid_login
        gateway = PagarmeGateway.new(username: '')
