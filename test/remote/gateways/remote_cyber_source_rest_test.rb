...
class RemoteCyberSourceRestTest < Test::Unit::TestCase
  ...
  def test_failure_refund
    purchase = @gateway.purchase(@amount, @card_with_out_funds, @options)
    response = @gateway.refund(@amount, { order_id: purchase.authorization })
  ...
  def test_successful_less_than_the_payment_amount_refund
    purchase = @gateway.purchase(@amount, @visa_card, @options)
    response = @gateway.refund(@amount / 2, { order_id: purchase.authorization })
  ...
  def test_successful_double_refund_transactions
    purchase = @gateway.purchase(@amount, @visa_card, @options)
    response1 = @gateway.refund(@amount, { order_id: purchase.authorization })
    ...
    response2 = @gateway.refund(@amount, { order_id: purchase.authorization })
  ...
  def test_failure_credit
    response = @gateway.credit(@amount, @card_with_out_funds, @options)
