...
class RemoteOgoneTest < Test::Unit::TestCase
  ...
  def test_successful_purchase_with_3d_secure_v2_with_d3d
    ...
    assert_equal OgoneGateway::SUCCESS_MESSAGE, response.message
    html_answer = response.params['HTML_ANSWER']
    assert html_answer
    assert_match 'v2-sim', Base64.decode64(html_answer)
    creq_regex = /name="creq" value="(.*?)"/
    creq = Base64.decode64(html_answer).match(creq_regex)[1]
    assert_equal  JSON.parse(Base64.decode64(creq))['messageVersion'], '2.2.0' # 3DS version
    assert_equal  JSON.parse(Base64.decode64(creq))['messageType'], 'CReq' # Challenge Request
  end

  def test_successful_purchase_with_3d_secure_v2_with_execute_threed
    options = @options_browser_info.merge(execute_threed: true)
  ...
  def test_successful_purchase_with_3d_secure_v2_with_3ds_v1_card
  ...
  def test_successful_purchase_with_3d_secure_v2_frictionless
    assert response = @gateway_3ds.purchase(@amount, @credit_card_d3d_2_frictionless, @options_browser_info.merge(execute_threed: true))
    assert_success response
  ...
  def test_successful_purchase_with_3d_secure_v2_recomended_parameters
    ...
    assert response = @gateway_3ds.authorize(@amount, @credit_card_d3d_2_challenge, options.merge(execute_threed: true))
    assert_success response
    assert_equal '46', response.params['STATUS']
    assert_equal OgoneGateway::SUCCESS_MESSAGE, response.message
    html_answer = response.params['HTML_ANSWER']
  ...
  def test_unsuccessful_purchase_with_3d_secure_v2
    ...
    assert response = @gateway_3ds.purchase(@amount, @credit_card_d3d_2_challenge, @options_browser_info.merge(execute_threed: true))
