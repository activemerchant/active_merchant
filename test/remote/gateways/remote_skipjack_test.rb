...
class RemoteSkipJackTest < Test::Unit::TestCase
  def setup
    ...
    @creditcard = credit_card('4445999922225',
  ...
  def test_successful_authorization
    authorization = @gateway.authorize(100, @creditcard, @options)
  ...
  def test_successful_purchase
    assert_success @gateway.purchase(100, @creditcard, @options)
  end
  ...
  def test_successful_authorization_and_capture
    ...
    capture = @gateway.capture(100, authorization.authorization)
    assert_success capture
  end
  
  def test_authorization_and_void
    authorization = @gateway.authorize(101, @creditcard, @options)
    assert_success authorization
  ...
  def test_successful_authorization_and_credit
    ...
    capture = @gateway.capture(100, authorization.authorization, :force_settlement => true)
    assert_success capture 

    # developer login won't change transaction immediately to settled, so status will have to mismatch
    credit = @gateway.credit(100, authorization.authorization)
