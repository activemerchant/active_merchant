...
class RemoteVantivExpressTest < Test::Unit::TestCase
  ...
  def test_successful_purchase_with_level_3_data
    ...
    response = @gateway.purchase(@amount, @credit_card, @options.merge(level_iii_options))
    assert_success response
    assert_equal 'Approved', response.message
  end

  def test_successful_purchase_with_level_3_and_lodging_data
    extended_parameters = {
      lodging: {
        agreement_number: SecureRandom.hex(12),
        check_in_date: 20250910,
    ...
    response = @gateway.purchase(@amount, @credit_card, @options.merge(extended_parameters))
    assert_success response
    assert_equal 'Approved', response.message
  end
  ...
  def test_successful_authorize_and_capture_with_level_3_data
    ...
    auth = @gateway.authorize(@amount, @credit_card, @options.merge(level_iii_options))
    assert_success auth
