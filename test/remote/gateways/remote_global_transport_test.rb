...
class RemoteGlobalTransportTest < Test::Unit::TestCase
  ...
  def test_failed_capture
    ...
    assert capture = @gateway.capture(@amount + 500, auth.authorization)
  ...
  def test_partial_refund
    ...
    assert refund = @gateway.refund(@amount - 10, purchase.authorization)
    assert_success refund
    assert_equal "Approved", refund.message
  end
  ...
  def test_failed_refund
    ...
    assert refund = @gateway.refund(@amount + 500, purchase.authorization)
