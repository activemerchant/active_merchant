  ...
  module Billing #:nodoc:
    class VersaPayGateway < Gateway
      ...
      def unstore(authorization, options = {})
        ...
        commit('wallets', {}, :delete, { fund_token: fund_token, wallet_token: wallet_token })
      end
      ...
      def commit(action, post, method = :post, options = {})
        raw_response = ssl_request(method, url(action, method, options), post.to_json, request_headers)
      ...
      def url(endpoint, method, options = {})
        case endpoint
        when 'wallets'
          parameters = method == :delete ? "/#{options[:wallet_token]}/methods/#{options[:fund_token]}" : ''
