  ...
  module Billing #:nodoc:
    class VersaPayGateway < Gateway
      ...
      def transact(money, payment, options = {}, type = 'sale')
        post = {
          amount_cents: money,
          contact: { email: options&.dig(:email) }
        }.compact
      ...
      def add_order(post, money, options = {})
        ...
        add_attributes(order, options[:attributes])
        add_order_items(order, options)
        post[:order] = order
      end

      def add_attributes(hash, attributes)
        return unless attributes

        attributes.each_with_index do |atr, i|
          hash[:"order_att#{1 + i}"] = atr
        end
      end

      def add_order_items(post, options)
        return unless options[:order_items]

        post[:order_items] = options[:order_items].map.with_index(1) do |order_item, i|
          {
            number: i,
            description: order_item[:description],
            quantity: order_item[:quantity],
            unit_cost_cents: order_item[:unit_cost_cents],
            amount_cents: order_item[:amount_cents],
            tax_amount_cents: order_item[:tax_amount_cents],
            discount_amount_cents: order_item[:discount_amount_cents],
            product_code: order_item[:product_code],
            category: order_item[:category]
          }.compact.tap do |order_it|
            add_attributes(order_it, order_item[:attributes])
          end
        end
      end
      ...
      def parse(body)
        ...
        data.each do |key, _value|
          next unless key.is_a?(String) && key.strip.start_with?('{')

          begin
            parsed_key = JSON.parse(key).with_indifferent_access
            data.merge!(parsed_key)
            data.delete(key)
      ...
      def message_from(response)
        ...
        success_from(response) ? 'Succeeded' : response_message.map { |key, value| "#{key}: #{value}" }.join(' | ')
      end
