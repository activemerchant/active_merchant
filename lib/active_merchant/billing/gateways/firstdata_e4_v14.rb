module ActiveMerchant #:nodoc:
  module Billing #:nodoc:
    class FirstdataE4V14Gateway < Gateway
      # TransArmor support requires v14 or higher than v12
      self.test_url = 'https://api.demo.globalgatewaye4.firstdata.com'
      self.live_url = 'https://api.globalgatewaye4.firstdata.com'
      ...
      # * <tt>:key_id</tt>     -- The HMAC key ID.
      # * <tt>:key_secret</tt> -- The HMAC key secret.
      def initialize(options = {})
        requires!(options, :login, :password, :key_id, :key_secret)
        @options = options
        super
      end
      ...
      def add_credit_card_verification_strings(xml, credit_card, options)
        ...
        if address
          ...
          xml.tag! "Address", address_values.join("|")
        end
      ...
      def commit(action, request, credit_card = nil)
        ...
        begin
          ...
          response = parse(ssl_post(URI.join(url, URN), request_body, post_headers(request_body)))
      ...
      def build_authorization_header(content_digest, time)
        data = ['POST', CONTENT_TYPE, content_digest.to_s, time, URN].join("\n")
        hmac = Base64.strict_encode64(OpenSSL::HMAC.digest(content_digest, @options[:key_secret], data))
        "GGE4_API #{@options[:key_id]}:#{hmac}"
      end


      def post_headers(request_body)
        content_digest = OpenSSL::Digest.new('sha1', request_body)
        time = Time.now.utc.iso8601
        { 'Authorization' => build_authorization_header(content_digest, time),
          'X-GGe4-Date' => time,
