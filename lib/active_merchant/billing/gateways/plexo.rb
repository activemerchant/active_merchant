module ActiveMerchant # :nodoc:
  module Billing # :nodoc:
    class PlexoGateway < Gateway
      ...
      def build_url(action, base, options, parameters)
        return "#{base}/#{parameters[:payment_id]}" if action == 'status' && parameters[:payment_id]
      ...
      def commit(action, parameters, options = {})
        ...
        url = build_url(action, base_url, options, parameters)
