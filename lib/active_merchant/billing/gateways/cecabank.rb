module ActiveMerchant #:nodoc:
  module Billing #:nodoc:
    class CecabankGateway < Gateway
      ...
      #Amounts is passed as an integer, being the last 2 positions it's cents: 12.34 => 1234
      ...
      def purchase(money, creditcard, options = {})
        post = {'Descripcion' => options[:description], 
                ...
                'Importe' => money, 
                'TipoMoneda' => CECA_CURRENCIES_DICTIONARY[options[:currency] || default_currency]}

        add_creditcard(post, creditcard)

        commit(CECA_ACTION_PURCHASE, money, post)
      end
      ...
      def refund(money, identification, options = {})
        post = {'Referencia' => identification, 
                'Num_operacion' => options[:order_id], 
        ...
        commit(CECA_ACTION_REFUND, money, post)
      end      
      ...
      def parse(body)
        ...
        if root.elements['ERROR']
          ...
          if "000" == root.elements['OPERACION'].attributes['numeroOperacion']
            response[:authorization] = root.elements['OPERACION/numeroAutorizacion'] ? root.elements['OPERACION/numeroAutorizacion'].text : nil
          else
            response[:authorization] = root.attributes['numeroOperacion'] ? root.attributes['numeroOperacion'] : nil 
          end
      ...
      def commit(action, money, parameters)
        parameters.merge!(
            {
              ...
              'Firma' => generate_signature(action, money, parameters),
        ...
        xml = ssl_post("#{url}/cgi-bin/#{action}", post_data(parameters))
        response = parse(xml)
        Response.new(response[:success], 
                     response[:message], 
                     response,
                     :test => test?, :authorization => response[:operation_number] )
      end
      ...
      def generate_signature(action, money, parameters)
        signature_fields = if action == CECA_ACTION_REFUND
                             options[:key].to_s + options[:merchant_id].to_s + options[:acquirer_bin].to_s +
                             options[:terminal_id].to_s + parameters['Num_operacion'].to_s + parameters['Importe'].to_s +
                             parameters['TipoMoneda'].to_s + CECA_DECIMALS + parameters['Referencia'].to_s + CECA_ENCRIPTION
                           else
                             options[:key].to_s + options[:merchant_id].to_s + options[:acquirer_bin].to_s +
                             options[:terminal_id].to_s + parameters['Num_operacion'].to_s + parameters['Importe'].to_s +
                             parameters['TipoMoneda'].to_s + CECA_DECIMALS + CECA_ENCRIPTION + CECA_NOTIFICATIONS_URL + CECA_NOTIFICATIONS_URL
                           end
