    ...
    class MerchantWarriorGateway < Gateway
      ...
      def authorize(money, credit_card_or_token, options = {})
        ...
        if credit_card_or_token.kind_of? CreditCard
          ...
          card_id, card_key, key_replace = credit_card_or_token.split(";")
          ...
          post.merge!('transactionAmount' => sprintf("%.2f",money))
      ...
      def capture(money, identification)
        post = {}
        post.merge!('transactionID' => identification)
        post.merge!('transactionAmount' => money.to_s)
      ...
      def add_customer_data(post, options)
        post.merge!('customerName' => (options[:address][:name] if  options[:address]))
      end

      def add_address(post, options)
        post.merge!('customerCountry' => (options[:address][:country] if options[:address]) ,
                    'customerState' => (options[:address][:state] if options[:address]) ,
                    'customerCity' => (options[:address][:city] if options[:address]) ,
                    'customerAddress' => (options[:address][:address1] if options[:address]),
                    'customerPostCode' => (options[:address][:zip] if options[:address]))
      end
