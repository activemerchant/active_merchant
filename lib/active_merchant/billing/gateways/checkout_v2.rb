  ...
  module Billing #:nodoc:
    class CheckoutV2Gateway < Gateway
      ...
      def response(action, succeeded, response)
        successful_response = succeeded && action == :purchase || action == :authorize || action == :instruments
      ...
      def headers(action)
        {
          'Authorization' => action == :tokens ? @options[:public_key] : @options[:secret_key],
      ...
      def authorization_from(action, raw)
        if action == :tokens
          raw['token']
        else
          raw['id']
