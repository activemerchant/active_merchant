  ...
  module Billing # :nodoc:
    class NetbanxGateway < Gateway
      ...
      def commit(method, uri, parameters)
        begin
          params = parameters.nil? || method == :get ? nil : parameters.to_json
          response = parse(ssl_request(method, get_url(uri), params, headers))
      ...
      def error_code_from(response)
        unless success_from(response)
          ...
          when '3004'
            STANDARD_ERROR_CODE[:incorrect_zip]
          when '3005'
            STANDARD_ERROR_CODE[:incorrect_cvc]
          when '3006'
            STANDARD_ERROR_CODE[:expired_card]
          when '3009', '3011'..'3016', '3022'..'3032', '3035'..'3042', '3046'..'3054'
