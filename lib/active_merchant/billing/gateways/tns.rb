module ActiveMerchant #:nodoc:
  module Billing #:nodoc:
    class TnsGateway < Gateway
      ...
      self.supported_countries = %w(AL AR AU AZ BE GA BR BG KH CA CL CH CZ EG EE FR DE GR GT HK HU IN ID IL IT JP KR KW LV LB LT LU MK MY MX MD MA MM NL NC NZ NG PH PL PT RO RU SA SG SK ZA ES LK SE CH TW TH TR UA AE GB US VN)
      ...
      def add_customer_data(post, options)
        ...
        if(billing_address = (options[:billing_address] || options[:address]))
          ...
          customer[:ipaddress]         = billing_address[:ip]
        end
      ...
      def split_authorization(authorization)
        orderid, transactionid = authorization.split('|')
      end
      ...
      def split_name(full_name)
        ...
        last_name, first_middle_names = [names.pop, names.join(' ')]
      end
    end
  end
end
