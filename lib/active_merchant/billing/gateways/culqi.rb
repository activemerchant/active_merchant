...
module ActiveMerchant #:nodoc:
  module Billing #:nodoc:
    class CulqiGateway < Gateway
      ...
      # Purchase and Authorize calls are ditto as per Culqi API.
      # Merchant account is pre-configured by Culqi for
      # one step (purchase) or two step (auth+capture) transaction.
      #
      ...
      def add_checksum(action, post)
        ...
        when :invalidate
          ...
          checksum_array = [
            ...
            post[:description],
            post[:redirecturl],
            post[:cardnumber] || post[:token],
            @options[:secret_key]
          ]
        end
        post[:checksum] = Digest::MD5.hexdigest(checksum_array.compact.join('|'))
      end
