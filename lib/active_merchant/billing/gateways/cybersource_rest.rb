require 'active_merchant/billing/gateways/cybersource/cybersource_common'
  ...
  module Billing #:nodoc:
    class CybersourceRestGateway < Gateway
      ...
      def commit(action, post)
        ...
        Response.new(false, response['response']['rmsg'], response, test: test?)
      end
      ...
      def get_http_signature(resource, digest, http_method = 'post', gmtdatetime = Time.now.httpdate)
        string_to_sign = {
          host: URI.parse(test? ? test_url : live_url).host,
          date: gmtdatetime,
