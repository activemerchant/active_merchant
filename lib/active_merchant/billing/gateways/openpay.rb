  ...
  module Billing #:nodoc:
    ...
    class OpenpayGateway < Gateway
      ...
      def store(creditcard, options = {})
        ...
        if options[:customer].present?
          ...
          MultiResponse.run(:first) do |r|
            ...
            if r.success? and !r.params['id'].blank?
      ...
      def add_address(card, options)
        ...
        if opt_address = options[:billing_address] || options[:address]
          address = {
              :line1 => opt_address[:address1],
              :line2 => opt_address[:address2],
              :line3 => opt_address[:company],
              :city => opt_address[:city],
              :postal_code => opt_address[:zip],
              :state => opt_address[:state],
              :country_code => opt_address[:country]
          }
      ...
      def commit(method, resource, parameters, options = {})
        ...
        Response.new(success,
                     success ? nil : response['description'],
                     response,
                     :test => !@production,
