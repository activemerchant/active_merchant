  ...
  module Billing #:nodoc:
    class BanwireGateway < Gateway
	  ...
	  def add_order_data(post, money, options)
		...
		post[:ORD_CONCEPT] = options[:description] #- Concept (required) [alphanumeric - lenght: 1-33] (Example: Compra de articulos)
	  end
	  ...
	  def add_special_data(post, options)
		...
		post[:CUST_IP] = options[:ip] if options[:ip] #- IP address of customer (required) [text - lenght: 255] (Example: 127.0.0.1)
		post[:ebWEBSITE] = options[:web_site] if options[:web_site] #- URL website (required) [text - lenght: 1-60] (Example: www.comercio.com)
		...
		post[:NOTIFY_URL] = options[:notify_url] if options[:notify_url] #- Notify URL (required) [text - lenght: 255] (Example: www.comercio.com/response.php)
