...
module ActiveMerchant #:nodoc:
  module Billing #:nodoc:
    ...
    class StripePaymentIntentsGateway < StripeGateway
      ...
      ALLOWED_CANCELLATION_REASONS = %w[
        duplicate
        fraudulent
        requested_by_customer
        abandoned
      ].freeze
      CREATE_INTENT_ATTRIBUTES = %i[
        description
        statement_descriptor
        receipt_email
        save_payment_method
      ]
      CONFIRM_INTENT_ATTRIBUTES = %i[
        receipt_email
        return_url
        save_payment_method
        setup_future_usage
      ...
      def commit(method, url, parameters = nil, options = {})
        ...
        avs_code =
          AVS_CODE_TRANSLATOR[
            "line1: #{card['address_line1_check']}, zip: #{card['address_zip_check']}"
          ]
        ...
        Response.new(
          ...
          if success
            ...
            response.dig('last_payment_error', 'message') || response['status']
          end,
          response,
      ...
      def authorization_from(success, url, method, response)
        unless success || response['last_payment_error'].nil?
          return response['last_payment_error']['charge']
        end

        if url == 'customers'
          [
            response['id'],
            response.dig('sources', 'data').first&.dig('id')
          ].join('|')
        elsif method == :post &&
              (
                url.match(%r{customers\/.*\/cards}) ||
                  url.match(%r{payment_methods\/.*\/attach})
              )
      ...
      def emv_authorization_from_response(response)
        if response['last_payment_error']
          return response['last_payment_error']['emv_auth_data']
        end
      ...
      def error_code_from(response)
        return if response['last_payment_error'].nil?

        code = response['last_payment_error']['code']
        decline_code = response['last_payment_error']['decline_code'] if code ==
          'card_declined'
      ...
      def capture(money, intent_id, options = {})
        ...
        post[:application_fee_amount] = options[:application_fee] if options[
          :application_fee
        ]
      ...
      def void(intent_id, options = {})
        ...
        post[:cancellation_reason] =
          options[:cancellation_reason] if ALLOWED_CANCELLATION_REASONS
          .include?(options[:cancellation_reason])
      ...
      def unstore(identification, options = {}, deprecated_options = {})
        if identification.include?('pm_')
          ...
          commit(
            :post,
            "payment_methods/#{payment_method}/detach",
            nil,
            options
          )
        else
      ...
      def add_card_data(post, payment_method)
        ...
        post[:card][:cvc] = payment_method.verification_value if payment_method
          .verification_value
        post
      end
      ...
      def add_capture_method(post, options)
        ...
        post[:capture_method] = capture_method if ALLOWED_METHOD_STATES
          .include?(capture_method)
        post
      end

      def add_confirmation_method(post, options)
        confirmation_method = options[:confirmation_method].to_s
        post[:confirmation_method] =
          confirmation_method if ALLOWED_METHOD_STATES.include?(
          confirmation_method
        )
        post
      end
      ...
      def add_payment_method_types(post, options)
        payment_method_types = options[:payment_method_types] if options[
      ...
      def add_exemption(post, options = {})
        ...
        post[:payment_method_options][:card][:mit_exemption] ||=
          { claim_without_transaction_id: true } if options[:mit]
      end
      ...
      def setup_future_usage(post, options = {})
        post[:setup_future_usage] = options[:setup_future_usage] if %w[
          on_session
          off_session
        ].include?(options[:setup_future_usage])
        post[:off_session] = options[:off_session] if options[:off_session] &&
      ...
      def add_connected_account(post, options = {})
        ...
        post[:transfer_data][:amount] = options[:transfer_amount] if options[
          :transfer_amount
        ]
        post[:on_behalf_of] = options[:on_behalf_of] if options[:on_behalf_of]
        post[:transfer_group] = options[:transfer_group] if options[
          :transfer_group
        ]
        post[:application_fee_amount] = options[:application_fee] if options[
          :application_fee
      ...
      def add_billing_address(post, options = {})
        ...
        post[:billing_details][:address][:city] = billing[:city] if billing[
          :city
        ]
        post[:billing_details][:address][:country] =
          billing[:country] if billing[:country]
        post[:billing_details][:address][:line1] =
          billing[:address1] if billing[:address1]
        post[:billing_details][:address][:line2] =
          billing[:address2] if billing[:address2]
        post[:billing_details][:address][:postal_code] =
          billing[:zip] if billing[:zip]
        post[:billing_details][:address][:state] = billing[:state] if billing[
          :state
        ]
      ...
      def add_shipping_address(post, options = {})
        ...
        post[:shipping][:address][:city] =
          shipping[:address][:city] if shipping[:address][:city]
        post[:shipping][:address][:country] =
          shipping[:address][:country] if shipping[:address][:country]
        post[:shipping][:address][:line2] =
          shipping[:address][:line2] if shipping[:address][:line2]
        post[:shipping][:address][:postal_code] =
          shipping[:address][:postal_code] if shipping[:address][:postal_code]
        post[:shipping][:address][:state] =
          shipping[:address][:state] if shipping[:address][:state]
        ...
        post[:shipping][:tracking_number] =
          shipping[:tracking_number] if shipping[:tracking_number]
        post
      end
    end
  end
