module ActiveMerchant #:nodoc:
  module Billing #:nodoc:
    class CommerceHubGateway < Gateway
      ...
      def add_reference_transaction_details(post, authorization, options)
      ...
      def add_google_pay(source, options)
        source[:sourceType] = 'GooglePay'
        source[:data] = {}
        source[:data][:encryptedMessage] = options[:google_pay][:encrypted_message] if options[:google_pay][:encrypted_message]
        source[:data][:ephemeralPublicKey] = options[:google_pay][:ephemeral_public_key] if options[:google_pay][:ephemeral_public_key]
        source[:data][:tag] = options[:google_pay][:tag] if options[:google_pay][:tag]
        source[:signature] = options[:google_pay][:signature] if options[:google_pay][:signature]
        source[:version] = options[:google_pay][:version] if options[:google_pay][:version]
        source[:merchantId] = options[:google_pay][:merchant_id] if options[:google_pay][:merchant_id]
        source[:merchantPrivateKey] = options[:google_pay][:merchant_private_key] if options[:google_pay][:merchant_private_key]
        source[:signingVerificationKey] = options[:google_pay][:signing_verification_key] if options[:google_pay][:signing_verification_key]
      end

      def add_apple_pay(source, options)
        source[:sourceType] = 'ApplePay'
        source[:data] = options[:apple_pay][:data] if options[:apple_pay][:data]
        source[:signature] = options[:apple_pay][:signature] if options[:apple_pay][:signature]
        source[:applePayMerchantId] = options[:apple_pay][:merchant_id] if options[:apple_pay][:merchant_id]
        source[:version] = options[:apple_pay][:version] if options[:apple_pay][:version]
        source[:header] = {}
        source[:header][:applicationDataHash] = options[:apple_pay][:application_data_hash] if options[:apple_pay][:application_data_hash]
        source[:header][:ephemeralPublicKey] = options[:apple_pay][:ephemeral_public_key] if options[:apple_pay][:ephemeral_public_key]
        source[:header][:publicKeyHash] = options[:apple_pay][:public_key_hash] if options[:apple_pay][:public_key_hash]
        source[:header][:transactionId] = options[:apple_pay][:transaction_id] if options[:apple_pay][:transaction_id]
        source[:header][:applicationData] = options[:apple_pay][:application_data] if options[:apple_pay][:application_data]
        source[:header][:merchantPrivateKey] = options[:apple_pay][:merchant_private_key] if options[:apple_pay][:merchant_private_key]
      end
      ...
      def add_payment(post, payment, options = {})
        source = {}
        if payment.is_a?(NetworkTokenizationCreditCard) && payment.brand == 'google_pay' && options[:google_pay]
          add_google_pay(source, options)
        elsif payment.is_a?(NetworkTokenizationCreditCard) && payment.brand == 'apple_pay' && options[:apple_pay]
          add_apple_pay(source, options)
