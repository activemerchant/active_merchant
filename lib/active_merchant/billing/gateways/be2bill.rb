  ...
  module Billing #:nodoc:
    class Be2billGateway < Gateway
      ...
      def add_customer_data(post, options)
        ...
        post[:CLIENTIDENT]     = options[:client_id]
      end
      ...
      def add_alias(post, options)
        if options.keys.include?(:aliasmode) or options.keys.include?('aliasmode')
          post[:ALIAS]     = options[:alias]
          post[:ALIASMODE] = options[:aliasmode]
        else
          post[:CREATEALIAS] = 'yes'
        end
      end
      ...
      def successful?(response)
        response['EXECCODE'].in? ['0000', '0001']
      end
      ...
      def post_data(action, parameters = {})
        parameters[:HASH] = signature(parameters, action)
        { :method => action,
          :params => parameters }.to_query
      end

      def signature(parameters, action)
        parameters[:OPERATIONTYPE] = action if not parameters[:OPERATIONTYPE]
