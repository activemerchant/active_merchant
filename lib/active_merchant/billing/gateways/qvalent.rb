module ActiveMerchant #:nodoc:
  module Billing #:nodoc:
    class QvalentGateway < Gateway
      ...
      def eci(options)
        return 'SSL' if options.dig(:stored_credential, :initial_transaction)

        if options.dig(:stored_credential, :initiator) && options[:stored_credential][:initiator] == 'cardholder'
          'MTO'
        elsif type = options.dig(:stored_credential, :reason_type)
          REASON_TYPE[type]
        else
          'SSL'
        end
