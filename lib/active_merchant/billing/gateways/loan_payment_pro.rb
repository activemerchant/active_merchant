  ...
  module Billing # :nodoc:
    class LoanPaymentProGateway < Gateway
      ...
      def add_address(post, payment_method, options)
        ...
        post[:BillingFirstName] = first_name
        post[:BillingLastName] = last_name
        post[:BillingAddress1] = address[:address1]
        post[:BillingAddress2] = address[:address2]
        post[:BillingCity] = address[:city]
        post[:BillingState] = address[:state]
        post[:BillingZip] = address[:zip]
      end
      ...
      def add_payment(post, payment)
        ...
        when CreditCard
          ...
          post[:CardCode] = payment.verification_value
          post[:ExpMonth] = format(payment.month, :two_digits)
          post[:ExpYear] = format(payment.year, :two_digits)
        end
      end
      ...
      def authorization_from(response = {}, post = {})
        return response[:PaymentMethodToken] if response[:PaymentMethodToken].present?

        [response[:TransactionId], post[:InvoiceId]].compact.join('|')
      end
