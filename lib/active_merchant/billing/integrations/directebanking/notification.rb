    ...
    module Integrations #:nodoc:
      module Directebanking
        class Notification < ActiveMerchant::Billing::Integrations::Notification
          ...
          def status
            # Notifications: 
            #  Please pay attention that you are only notified about successful transactions.
          ...
          def generate_signature_string
            sig_str = PAYMENT_HOOK_SIGNATURE_FIELDS.map {|key| params[key.to_s]} * "|"+ "|"+@options[:credential4]
          ...
          def signature_is_valid?
            #puts "signature_is_valid? #{generate_signature.to_s}==#{params['hash'].to_s}"
